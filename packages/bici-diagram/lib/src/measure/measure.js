export function biciMeasure(ctx, node) {
    ...
    var vh = node.rect.height * (value - dataMin) / (dataMax - dataMin);
    ...
    dataColors = dataColors.filter(function (item) { return item.checked == true; });
...
var radiusRect = function (ctx, left, top, width, height, r) {
    var pi = Math.PI;
    ctx.beginPath();
