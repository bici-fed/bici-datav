export function calcCanvas(width, height) {
    if (width === void 0) { width = 826; }
    if (height === void 0) { height = 1168; }
    var minWidth = 3198;
    var minHeight = 2288;
    var left = 1186;
    var top = 560;
...
export function calcScroll(width, height) {
    if (width === void 0) { width = 826; }
    if (height === void 0) { height = 1168; }
}
export function getHexColor(color) {
    ...
    var a = parseFloat(color.a || 1), r = Math.floor(a * parseInt(color.r) + (1 - a) * 255), g = Math.floor(a * parseInt(color.g) + (1 - a) * 255), b = Math.floor(a * parseInt(color.b) + (1 - a) * 255);
    return ('#' +
...
export function roundFun(value, n) {
    ...
    var s = (Math.round(value * Math.pow(10, n)) / Math.pow(10, n));
    // let rs = s.indexOf('.');
    // if (rs < 0) {
    //   s += '.';
    // }
...
export function eraseOverlapIntervals(intervals) {
    intervals.sort(function (a, b) { return a[1] - b[1]; }); //按照区间末位对这些区间排个位，保证结束时间是按序上升的，从前往后取总是能取到当前结束时间的最小值
    // 重叠的区间
    var res = [];
