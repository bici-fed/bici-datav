...
const VideoPlayer = (props) => {
  ...
  const { videoObj, showChannel, removeVideo, updateVideoUrl, showChannelSelect } = props
...
// 重新推流
  ...
  return (
    <div className={styles.videoContainer} ref={wrapperRef}>
      ...
      {showChannel && !hasVideo && (
        <span className={styles.channelIcon} style={{ color: videoObj?.id && !videoObj?.flv ? 'red' : 'white' }}>
          {/* <ZYTGIcon type="iconshipinjiankong" style={{ fontSize: 32, marginBottom: 5 }} /> */}
          {videoObj?.id ? (videoObj?.flv ? '暂无视频画面,请刷新重试' : '请选择通道重新登录') : '暂未绑定通道'}
        </span>
      )}
